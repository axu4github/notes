...
在文档发送给副本之前，分片代表会在本地创建索引文档。这是为了在将文档转送给副本之前，使用 `更新日志（tlog）` 验证文档并确保文档是安全持久地存储在系统里。此外，代表会给每个新文档分配一个 `版本号（Version ID）` 。对于现存文档，代表会将文档的当前的版本号与版本号作对比，以支持第5章提到的 `乐观锁（optimistic locking）` 过程。
